...
impl<A, B> MultiplexService<A, B> {
    pub fn new(rest: A, grpc: B) -> Self {
        Self { rest, rest_ready: false, grpc, grpc_ready: false }
    }
}
    ...
    B::Future: Send + 'static,
{
    ...
    fn call(&mut self, req: Request<Body>) -> Self::Future {
        ...
        assert!(self.grpc_ready, "grpc service not ready. Did you forget to call `poll_ready`?");
        assert!(self.rest_ready, "rest service not ready. Did you forget to call `poll_ready`?");
